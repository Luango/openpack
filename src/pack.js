  ...
  const spring = createSpring({
    ...
    onTick: (c) => {
      if (split) {
        ...
        pieceA.setAttribute("transform", `translate(${(dirA.x * d).toFixed(2)} ${(dirA.y * d).toFixed(2)})`);
        pieceB.setAttribute("transform", `translate(${(dirB.x * d).toFixed(2)} ${(dirB.y * d).toFixed(2)})`);
      } else if (tearPath) {
